...
fn parse_used_percent(
    ...
    if let Some(raw) =
        field(value, "remainingPercent", "remaining_percent").or_else(|| value.get("remaining"))
    {
        if raw.is_null() {
            return Ok(previous.map(|window| window.used_percent));
        }
...
fn parse_window(
    value: &Value,
    previous: Option<&RateLimitWindow>,
) -> Result<Option<RateLimitWindow>, String> {
    if !value.is_object() {
    ...
    let resets_at = optional_non_negative_integer(
        ...
        "resetsAt",
        "resets_at",
        previous.and_then(|window| window.reset_at),
    )?;
...
pub(crate) fn parse_rate_limits(
    ...
    let source = envelope
        .get("rateLimits")
        .or_else(|| envelope.get("rate_limits"))
        .unwrap_or(envelope);
    if !source.is_object() {
        return Err("rate limits must be an object".into());
    }
    let parse_kind = |camel: &str,
                      snake: &str,
    ...
    let plan_type = optional_string(
        source,
        "planType",
        "plan_type",
        previous.and_then(|snapshot| snapshot.plan_type.clone()),
    )?;
...
pub(crate) fn is_usage_limit_exceeded(error: &Value) -> bool {
    let candidate = error
        ...
        .or_else(|| {
            error
                .get("error")
                .and_then(|nested| nested.get("codexErrorInfo"))
        })
        .or_else(|| {
            error
                .get("error")
                .and_then(|nested| nested.get("codex_error_info"))
        });
...
mod tests {
    ...
    fn parses_full_json_rpc_rate_limits_response() {
        ...
        assert_eq!(snapshot.secondary.expect("secondary").reset_at, Some(3_000));
    ...
    fn sparse_snake_case_update_preserves_existing_window_fields_and_hard_limit() {
        let initial = parse_rate_limits(
            &json!({"primary":{"usedPercent":40,"resetsAt":42},"rateLimitReachedType":"hard"}),
            None,
            1,
        )
        .unwrap();
        let sparse =
            parse_rate_limits(&json!({"primary":{"used_percent":90}}), Some(&initial), 2).unwrap();
        assert_eq!(sparse.primary.unwrap().reset_at, Some(42));
    ...
    fn explicit_null_clears_hard_limit_only_when_present() {
        ...
        let cleared =
            parse_rate_limits(&json!({"rate_limit_reached_type":null}), Some(&initial), 2).unwrap();
    ...
    fn malformed_percentage_timestamp_and_observed_time_are_rejected() {
        ...
        assert!(parse_rate_limits(
            &json!({"primary":{"usedPercent":1,"resetsAt":"tomorrow"}}),
            None,
            1
        )
        .is_err());
    ...
    fn recognizes_terminal_usage_limit_in_both_protocol_shapes() {
        assert!(is_usage_limit_exceeded(
            &json!({"codexErrorInfo":"usageLimitExceeded"})
        ));
        assert!(is_usage_limit_exceeded(
            &json!({"error":{"codex_error_info":"usageLimitExceeded"}})
        ));
